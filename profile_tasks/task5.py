...
def get_model_instance_task5(model_name_str, pretrained=False, image_resolution_hw=(224, 224)):
    print(
        f"  Loading model: {model_name_str} (pretrained: {pretrained}) for resolution {image_resolution_hw}")
...
def get_profiling_dataloader_task5(data_root_path, batch_size, resolution_tuple_hw,
    ...
    dataset = None
    print(
        f"  Attempting to load Tiny ImageNet from: '{data_root_path}' for resolution {resolution_tuple_hw}")
    ...
    if not os.path.isdir(dataset_actual_path):
        print(
            f"  ERROR: Tiny ImageNet path '{dataset_actual_path}' is not a valid directory. Please provide the correct path.")
        return None
    try:
        full_dataset = datasets.ImageFolder(
            dataset_actual_path, transform=transform)
        if not full_dataset or len(full_dataset) == 0:
            print(
                f"  ERROR: Tiny ImageNet dataset at '{dataset_actual_path}' loaded as empty or failed to load.")
            return None
        if num_samples_to_use is not None and num_samples_to_use > 0 and num_samples_to_use < len(full_dataset):
            ...
            print(
                f"  Using the first {len(dataset)} samples from Tiny ImageNet ('{dataset_actual_path}').")
        else:
            dataset = full_dataset
            print(
                f"  Using all {len(dataset)} available samples from Tiny ImageNet ('{dataset_actual_path}').")
    except Exception as e:
        print(
            f"  ERROR: Failed to load Tiny ImageNet from '{dataset_actual_path}': {e}.")
        return None
    if dataset is None or len(dataset) == 0:
        return None  # Should be caught above
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False,
                            num_workers=num_workers, pin_memory=(num_workers > 0 and torch.cuda.is_available()))
    print(
        f"  DataLoader created: BS={batch_size}, NumBatches={len(dataloader)}, TotalSamplesInEpoch={len(dataset)}")
    return dataloader

def raw_draw(raw_data,save_file):
    ...
    if raw_data:
        # --- 演示提取 "CPU total" (平均值) ---
        for args in ["cpu_time","cuda_time","memory_usage","cuda_memory_usage","FLOPs","Parameters"]:
            Dict = get_raw_measure_dict_from_profiler_data(
            raw_data,
            args,
            average_over_calls=True
        )
            # print(Dict)
            display_extracted_measure_dict(Dict, args,
                                       is_averaged=True, unit_converter=simple_value_formatter,
                                       top_n=10,is_sort=True,save_file=save_file) # 显示前 10 个
...
# 保存到文件
    ...
    if args.trace_export:
        trace_file = f"{model_name_to_run}_{device.type}_batch{current_batch_size}_{224 if args.upper_simple else 64}.json"
